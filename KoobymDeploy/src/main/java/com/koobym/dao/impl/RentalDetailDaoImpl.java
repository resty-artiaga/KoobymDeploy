...
public class RentalDetailDaoImpl extends BaseDaoImpl<RentalDetail, Long> implements RentalDetailDao {
	...
	public List<RentalDetail> mostRentedBooks() {
		...
		flag = (List<RentalDetail>) session.createCriteria(RentalDetail.class).createAlias("bookOwner", "b")
	...
	public List<RentalDetail> suggestedBooksByGenre(int userId) {
		...
		Session session = getSessionFactory().getCurrentSession();		
		String squery = "select * from rental_detail  JOIN 	(select book_ownerId from book_owner join (SELECT bookId, count(genre_bookId) "
				+ "FROM genre_book JOIN genre_user ON genre_book.genreId = genre_user.genreId  WHERE genre_user.userId = :userId GROUP BY bookId ORDER BY 2 desc) "
				+ "as suggested_books "
				...
				+ "LEFT JOIN (select avg(rate.rateNumber) as rate, book_owner_rating.book_ownerId  as boi from rate JOIN book_owner_rating "
